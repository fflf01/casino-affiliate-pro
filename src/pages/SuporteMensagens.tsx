...
const initialMessages: Message[] = [
  {
    ...
    preview: "Olá, gostaria de saber como funciona o pagamento das comissões...",
    sender: "João Silva",
    date: "2024-01-25",
    status: "unread",
    priority: "high",
  },
  {
    ...
    preview: "Meu link não está rastreando os cliques corretamente...",
    sender: "Maria Santos",
    date: "2024-01-24",
    status: "unread",
    priority: "medium",
  },
  {
    ...
    preview: "Preciso de banners atualizados para divulgação...",
    sender: "Pedro Costa",
    date: "2024-01-23",
    status: "read",
    priority: "low",
  },
  {
    ...
    preview: "Gostaria de atualizar meus dados para recebimento...",
    sender: "Ana Oliveira",
    date: "2024-01-22",
    status: "replied",
    priority: "medium",
  },
  {
    ...
    preview: "Vi que tem uma promoção nova, como participar?",
    sender: "Carlos Lima",
    date: "2024-01-21",
    status: "read",
    priority: "low",
  },
  {
    ...
    preview: "Minha conta foi bloqueada sem motivo aparente...",
...
const SuporteMensagens = () => {
  ...
  const markAsRead = () => {
    if (selectedIds.length === 0) {
      toast({ title: "Selecione mensagens", description: "Nenhuma mensagem selecionada." });
      return;
    }
    setMessages((prev) =>
      prev.map((msg) =>
    ...
    toast({ title: "Marcadas como lidas", description: `${selectedIds.length} mensagem(ns) atualizada(s).` });
    setSelectedIds([]);
  };
  ...
  const markAsUnread = () => {
    ...
    toast({ title: "Marcadas como não lidas", description: `${selectedIds.length} mensagem(ns) atualizada(s).` });
    setSelectedIds([]);
  };
  ...
  const deleteSelected = () => {
    ...
    toast({ title: "Mensagens removidas", description: `${selectedIds.length} mensagem(ns) excluída(s).` });
    setSelectedIds([]);
  };
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <h1 className="text-3xl md:text-4xl font-display font-bold">
                Suas <span className="text-gradient-gold">Mensagens</span>
              </h1>
              <p className="text-muted-foreground mt-2">
                {unreadCount > 0 ? `${unreadCount} mensagem(ns) não lida(s)` : "Todas as mensagens foram lidas"}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="container mx-auto">
          ...
          <div className="bg-gradient-card rounded-2xl border border-border/50 p-4 mb-6">
            <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                <Button
                  variant={filter === "all" ? "neon" : "neonOutline"}
                  size="sm"
                ...
                <Button
                  variant={filter === "unread" ? "neon" : "neonOutline"}
                  size="sm"
                ...
                <Button
                  variant={filter === "read" ? "neon" : "neonOutline"}
                  size="sm"
                ...
                <Button
                  variant={filter === "replied" ? "neon" : "neonOutline"}
                  size="sm"
            ...
            {filteredMessages.length === 0 ? (
              <div className="bg-gradient-card rounded-2xl border border-border/50 p-12 text-center">
                ...
                <p className="text-muted-foreground">
                  {searchTerm ? "Tente ajustar sua busca" : "Você não tem mensagens nesta categoria"}
                </p>
              </div>
            ) : (
              filteredMessages.map((message) => (
                <div
                  ...
                  className={cn(
                    ...
                    message.status === "unread" && "border-l-4 border-l-primary bg-primary/5",
                    selectedIds.includes(message.id) && "ring-2 ring-primary/50 bg-primary/10"
                  )}
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-1">
                        ...
                        <Badge className={cn("w-fit text-xs", getPriorityColor(message.priority))}>
          ...
          {messages.length > 0 && (
            <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-gradient-card rounded-xl border border-border/50 p-4 text-center">
                <p className="text-2xl font-display font-bold text-primary">{messages.filter(m => m.status === "unread").length}</p>
                <p className="text-sm text-muted-foreground">Não lidas</p>
              </div>
              <div className="bg-gradient-card rounded-xl border border-border/50 p-4 text-center">
                <p className="text-2xl font-display font-bold text-foreground">{messages.filter(m => m.status === "read").length}</p>
                <p className="text-sm text-muted-foreground">Lidas</p>
              </div>
              <div className="bg-gradient-card rounded-xl border border-border/50 p-4 text-center">
                <p className="text-2xl font-display font-bold text-primary">{messages.filter(m => m.status === "replied").length}</p>
